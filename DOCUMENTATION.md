...
3. **Abstract Factory Pattern**: Manages `PackagingFactory` to generate matching families of packaging (`DineInPackagingFactory` vs. `TakeawayPackagingFactory`).
...
### When to Use
* When an order requires a matching set of items (e.g., Cup + Container) that must belong to the same style or category (Dine-In vs. Takeaway).
* When you need to prevent incompatible product combinations.

### Real-World Applications in This Project
* **Dine-In Order**: Requires washable, reusable tableware (`CeramicCup` + `GlassPlate`).
* **Takeaway Order**: Requires disposable packaging (`PaperCup` + `PaperBag`).

### Problem Scenario
If the cashier independently creates individual packaging items, mistakes can occur:
```python
cup = CeramicCup()       # Dine-In item
bag = PaperBag()         # Takeaway item (Incompatible combination!)
```
...
* **Concrete Factories (`DineInPackagingFactory`, `TakeawayPackagingFactory`)**: Produce matched sets of packaging.
...
class CeramicCup(Cup):
    def serve(self) -> str:
        return "Ceramic Cup (Dine-In)"

class GlassPlate(Container):
    def serve(self) -> str:
        return "Glass Plate (Dine-In)"

# Takeaway Family
class PaperCup(Cup):
    def serve(self) -> str:
        return "Disposable Paper Cup (Takeaway)"

class PaperBag(Container):
    def serve(self) -> str:
        return "Disposable Paper Bag (Takeaway)"
...
class TakeawayPackagingFactory(PackagingFactory):
    def create_cup(self) -> Cup:
        return PaperCup()
...
3. `TakeawayPackagingFactory` creates only disposable items (`PaperCup` + `PaperBag`).
...
* Eliminates the possibility of mixing takeaway and dine-in tableware.
...
```
==================================================================
        CAFE ORDER MANAGEMENT SYSTEM (3 DESIGN PATTERNS)          
==================================================================

>>> [Order #1] New Order: 'Cappuccino & Croissant' (Total: $8.50)
💳 Paid $8.50 using Credit/Debit Card.
📦 Packaging : Served in a Ceramic Cup (Dine-In) & Glass Plate (Dine-In)
✅ Order #1 Completed Successfully!

>>> [Order #2] New Order: 'Iced Latte & Muffin' (Total: $6.75)
💵 Paid $6.75 using Cash.
📦 Packaging : Served in a Disposable Paper Cup (Takeaway) & Disposable Paper Bag (Takeaway)
...
# 2203033 - Abhishek Bhattacharjee
